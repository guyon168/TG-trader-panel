"""OKX 现货撤单功能 — 修复 BUG-4: 撤单 instId 可能为 None"""
import logging
...
class OKXSpotCancelMixin:
    ...
    def cancel_orders_spot(self, symbol: Optional[str], order_id: Optional[str]) -> int:
        ...
        if order_id:
            ...
            if not inst_id:
                inst_id = self._find_inst_id_by_order_id_spot(order_id)
            ...
            open_orders = self.get_open_orders_spot(symbol)
    ...
    def _find_inst_id_by_order_id_spot(self, order_id: str) -> Optional[str]:
        """通过订单 ID 查找 instId (现货)"""
        # 1. 从成交记录反查（不需要 instId）
        try:
            res = self._trade_api.get_fills(ordId=order_id, instType='SPOT', limit='1')
        ...
        # 3. fallback: 遍历挂单列表
        try:
            orders = self.get_open_orders_spot(None)
            for o in orders:
